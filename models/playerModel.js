...
async function skillsHandler(arrays) {
	...
	const results = await Promise.allSettled([...deletePromises, createPromise, ...updatePromises]);

	results.forEach((result) => {
		if (result.status === "rejected") {
			console.error("Falha na operação:", result.reason.action, result.reason.error);
		}
	});
}
...
async function itensHandler(arrays) {
	...
	const results = await Promise.allSettled(...createPromise, ...deletePromise, ...updatePromises);
