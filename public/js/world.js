...
function actionForBtn(action) {
  switch (action) {
    case "fugir":
      if (
        dataPlayer.dexterity * dataPlayer.luck <
        opponentData.dexterity * opponentData.luck
      ) {
        txt.textContent = "Você não conseguiu fugir!";
        setTimeout(() => {
          openArena();
        }, 3000);
...
function skillToUse(data) {}

function mobAtack(mob) {
  while (statusPlayer > 0) {
    statusPlayer -= 20;
    /**
     * Criar função de randomizar a habilidade que o mob irá ultilizar para atacar o player cuja mesma irá retorna a habilidade
