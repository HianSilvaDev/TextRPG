...
function newCordinates(newx, newy) {
  ...
  if (x < 1 || x >= canvas.width || y < 0 || y >= canvas.height) {
    ...
    printNarration([{ text: "Fora dos limites!" }]);
    return;
  }
  ...
  console.log("x: " + cordinatesX, "y:" + cordinatesY);
  ...
  if (!dataCurrentRegion || (region != dataCurrentRegion.name && region)) {
    getDataRegion(region);
  }
  if (dataCurrentRegion) {
